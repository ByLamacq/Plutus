...
from ellipticcurve.math import add
...
def generate_private_key(): 
	...
	#return binascii.hexlify(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01').decode('utf-8').upper()
		...
		with open('plutus.txt', 'a') as file:
			file.write('hex private key: ' + str(private_key) + '\n' +
				   'WIF private key: ' + str(private_key_to_WIF(private_key)) + '\n' +
			           ...
			           'address: ' + str(address) + '\n\n')
	#else: 
		#print("Private key : "+str(private_key)+" -> Addess : "+str(address))
...
def main(database, cpu):
	...
	while True:
		...
		public_key, public_key_class = private_key_to_public_key(private_key)
		address = public_key_to_address(public_key)
		process(private_key, public_key, address, database)
		print("CPU : "+str(cpu)+" -> Private key : "+str(private_key)+" / Addess : "+str(address))
		while count>0:
			private_key = hex(int(private_key, 16) + 1) [2:]
			private_key = private_key.upper().zfill(64)		# ? seconds
			public_key, public_key_class = private_key_to_public_key_add(public_key_class, curve) # ? seconds
